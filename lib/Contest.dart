...
class _Contest_pay extends State<Contest_pay> {
  ...
  Future<List<dynamic>> loaddataPaiddata() async {
    final storage = LocalStorage('my_data');
    final token = await storage.getItem('jwt_token');
    final user_id = await storage.getItem('user_id');
    final response = await http.get(
      Uri.parse(
          '${dotenv.env['API_URL']}/api/paid_contest_list/${widget.id}'),
      headers: {
        'Content-type': 'application/json',
        'Accept': 'application/json',
        'Authorization': 'Bearer ${token}',
  ...
  late Future<List<dynamic>> futureAlbum;
  late Future<List<dynamic>> futureAlbumFor;
  ...
  void initState() {
    ...
    futureAlbum = loaddata();
    futureAlbumFor = loaddataPaiddata();
  }
